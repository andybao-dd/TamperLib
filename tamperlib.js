...
// @require      https://raw.githubusercontent.com/andybao-dd/TamperLib/refs/heads/master/tamperlib.js

const TamperLib = (function() {
    let TamperLib;

    class TamperLibStorage {
        static STORAGE_PREFIX = "tpl_";

        /**
         * Store a JS value at the supplied key. The supplied value must be JSON stringify-able.
         */
        setValue(key, value) {
            GM_setValue(TamperLibStorage.STORAGE_PREFIX + key, value);
        }

        /**
         * Get the value at the supplied key, returning the supplied default value if no value exists
         * at the key. The default value must be JSON stringify-able.
         */
        getValue(key, def) {
            return GM_getValue(TamperLibStorage.STORAGE_PREFIX + key, def);
        }
    }

    class TamperLibEvents {
        /**
         * Call the callback once the page is completely loaded (include all styles and images).
         * If the page is already completely loaded, the callback will be called immediately.
         * 
         * Alternatively call this function with no callback and instead it will return a promise that
         * will complete once the page is completely loaded.
         */
        onDocumentReady(callback) {
            if(callback == null) return new Promise(this.onDocumentReady);
    ...
    class TamperLibDocument {
        static WATCH_ALL_CHANGELIST = {
            subtree: true,
            childList: true,
            attributes: true,
        ...
        /**
         * Adds the supplied CSS to the page and returns the injected style element
         */
        addStyle(style) {
        ...
        /**
         * Call the callback when a node changes. If minWaitMs == 0, the callback will be passed
         * a list of mutations made to the node.
         *
         * Alternatively, call this function with no callback and instead it will return a promise.
         */
        waitForNodeChange(node, changeList = TamperLibDocument.WATCH_ALL_CHANGELIST, callback, minWaitMs = 250) {
            if (callback == null) return new Promise(resolve => {
                this.waitForNodeChange(node, changeList, resolve, minWaitMs);
            });

            let minTime = null;
            if (minWaitMs > 0) minTime = Date.now() + minWaitMs;

            let mo;
            mo = this.observeChanges(node, changeList, mutations => {
                mo.disconnect();
                if (minTime != null) {
                    const diffTime = minTime - Date.now();
                    if (diffTime > 0) {
        ...
        /**
         * Waits until a specific selector exists on a node
         * 
         * Alternatively, call this function with no callback and instead it will return a promise.
         */
        waitForSelector(node, selector, callback) {
            return this.waitForPredicate(node, {
                subtree: true,
                childList: true,
                attributes: true
            }, () => node.querySelector(selector) != null, callback);
        }

        /**
         * Observes changes of a node until the predicate is satisfied. You should also specify a list
         * of changes that will trigger the predicate. By default it will trigger the predicate on
         * all changes. The predicate will be called the mutations that occured since the last call
         * to the predicate.
         * 
         * Alternatively, call this function with no callback and instead it will return a promise.
         */
        waitForPredicate(node, changeList = TamperLibDocument.WATCH_ALL_CHANGELIST, predicate, callback) {
            if(callback == null) return new Promise(resolve => {
                this.waitForPredicate(node, changeList, predicate, resolve);
            });
        ...
        /**
         * Observe a node for changes. By default it will watch for all changes.
         * 
         * The callback will be called with the mutations that occured.
         * 
         * Returns the MutationObserver. You can call disconnect() on it when you are done.
         */
        observeChanges(node, changeList = TamperLibDocument.WATCH_ALL_CHANGELIST, callback) {
            const observer = new MutationObserver(callback);
            observer.observe(node, changeList);
            return observer;
        }
        ...
        /**
         * Returns { close, shadowRoot }
         * 
         * The overlay has `pointer-events: none` on it by default.
         * You need to manually specify `pointer-events: all` on clickable elements.
         */
        createOverlay(position = "fixed") {
            const element = document.createElement("div");
            element.style = `position: ${position}; top: 0; left: 0; width: 100%; height: 100%; z-index: 2147483647; pointer-events: none`;
            const shadowRoot = this.attachShadow(element);
            const style = document.createElement("style");
            style.textContent = `
            /* Theme v1 */
            :host {
                font-size: 10pt;
                --theme-bg-color: #140129;
                --theme-highlight-bg-color: #3b0293;
                --theme-text-color: #eeeeee;
                --theme-basic-color: #00ec4b;
                --theme-basic-dim-color: #00b93b;
                --theme-basic-accent-color: #8cd5b6;
                --theme-primary-color: #06aae0;
                --theme-secondary-color: #d7342a;
            ...
            a:visited {
                color: var(--theme-basic-dim-color);
            }
            a:hover, a:active {
         ...
         */
        createDialog(title, body, buttons = {}) {
            const { close, shadowRoot } = this.createOverlay();
            const { root, header, content, buttonsWrapper } = this.templateHTML(r => `
            <div ${r.root} style="display: flex; width: 100%; height: 100%; justify-content: center; align-items: center; background-color: rgba(0,0,0,0.6); pointer-events: all">
                <div style="border: 10px solid red; padding: 16px; max-width: 100%; max-height: 100%; background-color: var(--theme-bg-color); min-width: 300px">
                    <h1 ${r.header} style="margin-top: 0"></h1>
                    <div ${r.content}></div>
                    <div ${r.buttonsWrapper} style="padding-top: 16px; display: flex; gap: 8px; justify-content: right"></div>
                </div>
            </div>
            `);
            ...
            const buttonsWithClose = { ...buttons, "Close": close };
            for(const label of Object.keys(buttonsWithClose)) {
                const button = document.createElement("button");
                button.textContent = label;
                button.addEventListener("click", buttonsWithClose[label]);
         ...
         * Usage:
         * 
         * const { root, example } = templateHTML(r => `
         * <div ${r.root}>
         *     <span ${r.example}>Hi</span>
         * </div>
         * `);
         */
        templateHTML(htmlCreator) {
            const TEMPLATE_ATTR = "data-tamperlib-template-id";
            const refList = [];
            const refLookup = {};
            const refHandler = {
                get(target, prop, receiver) {
                    let idx = refLookup[prop];
                    if(idx == null) {
                        idx = refList.length;
                        refList.push(prop);
                        refLookup[prop] = idx;
                    }
                    return `${TEMPLATE_ATTR}="${idx}"`;
                }
            };
            const refObj = new Proxy({}, refHandler);
            const parser = new DOMParser();
            const parsedDOM = parser.parseFromString(htmlCreator(refObj), "text/html");
            const refs = {};
            for(const element of parsedDOM.querySelectorAll(`*[${TEMPLATE_ATTR}]`)) {
                const id = parseInt(element.dataset.tamperlibTemplateId);
                delete element.dataset.tamperlibTemplateId;
                const prop = refList[id];
                refs[prop] = element;
            }
            return refs;
        }
    ...
    class TamperLibPromises {
        /**
         * Returns a promise that completes in [millis] milliseconds.
         */
        sleep(millis) {
        ...
        /**
         * Returns a promise that waits a single document tick.
         */
        tick() {
            return this.sleep(1);
        }
    ...
    class TamperLibUtils {
        /**
         * Returns a function that debounces the input function. Function arguments are forwarded.
         * 
         * If maxDelay is non-null, debounce will wait no longer than maxDelay before calling the function.
         * 
         * Example usage:
         * const f = debounce(myFunc, 1000);
         * f();
         */
        debounce(func, delay, maxDelay = null) {
            let initialCallTime = null;
            let timeout = null;
            const callFunc = (args) => {
                initialCallTime = null;
                timeout = null;
                func(...args);
            };
            return (...args) => {
                ...
                const now = Date.now();
                if(initialCallTime == null) {
                    initialCallTime = now;
                }
                
                const timeTillMaxDelay = maxDelay != null ? (maxDelay - (now - initialCallTime)) : null;

                if(timeTillMaxDelay != null && timeTillMaxDelay < delay) {
                    timeout = setTimeout(callFunc, timeTillMaxDelay, args);
                } else {
                    timeout = setTimeout(callFunc, delay, args);
                }
            }
        }
    }

    class TamperLibNetwork {
    }

    const libs = {};
    function addLib(initer) {
        initer(libs);
    }

    TamperLib = {
        storage: new TamperLibStorage(),
        events: new TamperLibEvents(),
        document: new TamperLibDocument(),
        promises: new TamperLibPromises(),
        network: new TamperLibNetwork(),
        utils: new TamperLibUtils(),
        libs,
    };

    function markedLib(globalThis) {
/**
